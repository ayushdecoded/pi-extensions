...
- Those long completions also produce an in-terminal notification and a terminal-native desktop notification. Spark receives the task timeline with deterministic, tool-specific execution summaries—arguments, success/error state, counts, truncation, and compact output samples—while raw tool output and thinking stay excluded. It extracts a short outcome label classifying completion, input, blockage, or failure. The complete request is kept within an estimated 96k-token budget; unavailable, invalid, or slow generation falls back to a deterministic ready message. Desktop delivery uses `notify-send` on Linux, falling back to Kitty OSC 99 or OSC 777 when native delivery is unavailable.
